...
import report.model.SecurityInvoice;

/**
 *
 * @author Hiranya
 */
public class CashSecurityBill extends javax.swing.JPanel {
...
//    DecimalFormat df2 = new DecimalFormat("#,##0.00");
    ...
    public CashSecurityBill(JFrame parent) {
        initComponents();
    ...
    public void loadSequrityInvoice() {
        ...
        try {
            ...
            dtm.setRowCount(0);
            ResultSet rs;
            String query = "SELECT * FROM advance_invoice\n"
                    + "INNER JOIN lease ON advance_invoice.lease_uniqeid=lease.uniqeid\n"
                    + "INNER JOIN customer ON lease.customer_id=customer.cusid\n"
                    + "INNER JOIN pay_method ON advance_invoice.method_id=pay_method.id\n"
                    + " WHERE (`nic` LIKE '" + text + "%' OR `mobile` LIKE '" + text + "%' OR `fname` LIKE '" + text + "%' OR `lname` LIKE '" + text + "%' OR `lease`.`uniqeid` LIKE '" + text + "%' OR `advance_invoice`.`invoid` LIKE '" + text + "%'  )";
    ...
    SecurityInvoice secuInvoice = new SecurityInvoice();

    private void loadSecurityPayInfo() {
        ...
        try {
            ...
            String query = "SELECT *\n"
                    ...
                    + "LEFT JOIN advance_invoice ON lease.uniqeid=advance_invoice.lease_uniqeid "
            ...
            if (rs.next()) {
                ...
                String status;
                if (rs.getString("advance_invoice.invoid") == null) {
                ...
                this.secuInvoice.setLeaseId(leasId);
                this.secuInvoice.setCustomerName(cusname);
                this.secuInvoice.setMobile(rs.getString("customer.mobile"));
                this.secuInvoice.setEmail(rs.getString("customer.email"));
                this.secuInvoice.setSecurityAmount(Double.parseDouble(rs.getString("lease.advance_rate")));

                jTextField8.setText(rs.getString("lease_has_apartment.apartment_id"));
                jTextField4.setText(nic);
                jTextField5.setText(cusname);
                jTextField6.setText(df.format(Double.parseDouble(rs.getString("lease.advance_rate"))));
    ...
    private void addInvoice() {
        ...
        if (leaseid.isEmpty()) {
            ...
            Date today = new Date();
            long invoid = System.currentTimeMillis() - 680000000000L;

            this.secuInvoice.setDate(sdf.format(today));
            this.secuInvoice.setBalance(Double.parseDouble(balance));
            this.secuInvoice.setInvoid(String.valueOf(invoid));
            this.secuInvoice.setBarcode(String.valueOf(invoid));
            this.secuInvoice.setMethod(method);
            this.secuInvoice.setPayment(Double.parseDouble(payment));

            MySQL.iud("INSERT INTO advance_invoice (invoid,amount,balance,paydate,paytime,lease_uniqeid,method_id) \n"
                    + "VALUES ('" + invoid + "','" + payment + "','" + balance + "','" + sdf.format(today) + "','" + stf.format(today) + "','" + leaseid + "','" + paymentMethod.get(method) + "')");

            MySQL.iud("UPDATE lease SET `lease_status_id`='1'"
                    + "WHERE `uniqeid`='" + leaseid + "'");

            resetFeild();
            loadSequrityInvoice();
            printSecurityInvoice(this.secuInvoice);

        }
    }

    private void printSecurityInvoice(SecurityInvoice invoice) {

        String pathStream = "src/report/jasper/advance_invoice.jasper";
        String outputPath = "report/security_payment/" + invoice.getInvoid() + ".pdf";
        ...
        parameters.put("charge", df.format(invoice.getSecurityAmount()));
        ...
        parameters.put("balance", df.format(invoice.getBalance()));
...
//            JasperViewer.viewReport(jp, false);
    ...
    private void printSecurityInvoice() {

        String pathStream = "src/report/jasper/advance_invoice.jasper";
        String nic = jTable1.getValueAt(jTable1.getSelectedRow(), 2).toString();
        String invoicrId = jTable1.getValueAt(jTable1.getSelectedRow(), 0).toString();
        String outputPath = "report/security_payment/" + invoicrId + ".pdf";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                String charge = df.format(Double.parseDouble(payment) - Double.parseDouble(balance));
                String name = rs.getString("customer.fname") + " " + rs.getString("customer.lname");
                HashMap parameters = new HashMap();
                parameters.put("invoice", invoicrId);
                ...
                parameters.put("barcode", invoicrId);
...
//                JasperViewer.viewReport(jp, false);
    ...
    private void sendEmailAttachment(String email, String pdfFilePath, String fileName) {
        String sub = "Security Payment Receipt";
        String body = "We trust this message finds you well. We want to express our appreciation for your recent "
                + "payment of the security deposit for your apartment We have attached the official receipt for your records. "
                + "Please find it enclosed with this email.";
...
//            JOptionPane.showMessageDialog(this, "Email Sent Successfully", "Success", JOptionPane.INFORMATION_MESSAGE, StaticComponent.successIcon);
    ...
    private void initComponents() {
        ...
        jLabel4.setText("Security Payment History");
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Invoice ID", "Lease ID", "Customer NIC", "Method", "Date", "Payment", "Balance "
            }
        ) {
